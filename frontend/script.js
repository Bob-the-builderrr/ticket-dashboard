...
function setupMultiSelect(containerId, dropdownId, countId, stateKey) {
  ...
  const updateCount = () => {
    ...
    toggle.querySelector('span:first-child').textContent =
      count === 0 ? `Select ${stateKey === 'selectedPlans' ? 'Plan Types' : 'Categories'}`
...
function defaultDatesLast7() { const to = new Date(); const from = new Date(); from.setDate(to.getDate() - 6); id('from').value = toStr(from); id('to').value = toStr(to); }
function defaultDatesLast30() { const to = new Date(); const from = new Date(); from.setDate(to.getDate() - 29); id('from').value = toStr(from); id('to').value = toStr(to); }
function defaultDatesLast90() { const to = new Date(); const from = new Date(); from.setDate(to.getDate() - 89); id('from').value = toStr(from); id('to').value = toStr(to); }

async function loadPlans() {
  try {
    const res = await fetch(`${API_BASE}/api/plan-types`);
    ...
    dropdown.querySelectorAll('input[type="checkbox"]').forEach(cb => {
      cb.addEventListener('change', (e) => {
        if (e.target.id === 'plan-all') {
          if (e.target.checked) {
            state.selectedPlans = [];
            dropdown.querySelectorAll('input[type="checkbox"]').forEach(x => { if (x.id !== 'plan-all') x.checked = false; });
          }
        } else {
          if (e.target.checked) { state.selectedPlans.push(e.target.value); id('plan-all').checked = false; }
          else { state.selectedPlans = state.selectedPlans.filter(p => p !== e.target.value); }
        }
      ...
      cb.addEventListener('change', (e) => {
        if (e.target.id === 'category-all') {
          if (e.target.checked) {
            ...
            dropdown.querySelectorAll('input[type="checkbox"]').forEach(x => { if (x.id !== 'category-all') x.checked = false; });
          }
        } else {
          if (e.target.checked) { state.selectedCategories.push(e.target.value); id('category-all').checked = false; }
          else { state.selectedCategories = state.selectedCategories.filter(c => c !== e.target.value); }
        }
...
async function loadAll() {
  ...
  id('lastUpdated').textContent = 'Loading...';
  ...
  try {
    ...
    id('lastUpdated').textContent = `Last updated: ${new Date().toLocaleString()}`;
...
function renderLast() { if (!last.dates.length) return; renderChart(last.dates, last.colTotals); renderTable(); }
...
function renderChart(labels, data) {
  ...
  chart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Distinct Tickets per day', data, backgroundColor: 'rgba(59,130,246,0.7)', borderColor: 'rgba(59,130,246,1)', borderWidth: 1, borderRadius: 6, order: 2 },
        ...(trendData ? [{ label: 'Trend', data: trendData, type: 'line', borderColor: 'rgba(239,68,68,0.8)', borderWidth: 2, pointRadius: 0, fill: false, order: 1 }] : [])
      ]
    },
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,.08)' }, ticks: { color: '#cbd5e1', precision: 0 } }
      },
...
function formatDate(s){ const d=new Date(s); return d.toLocaleDateString('en-US',{month:'short',day:'numeric'}); }
